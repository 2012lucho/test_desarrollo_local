...
| `proyectos:update` | `{ id, nombre?, descripcion?, subproyectos?: Array<{ id?: number, nombre: string, descripcion?: string, tipo?: string, tecnologias?: number[] }>, componentes?: Array<{ nombre: string, descripcion?: string, config?: object | string, subproyectos?: number[] }>, tablas?: Array<{ nombre: string, pos_canvas_x?: number, pos_canvas_y?: number, campos?: Array<{ nombre: string, descripcion?: string }> }> }` | Actualiza un proyecto y reemplaza sus subproyectos, las tecnologías relacionadas, sus componentes, tablas y campos de tabla |
| `tablas:update-position` | `{ id, pos_canvas_x, pos_canvas_y }` | Actualiza la posición de una tabla dentro del canvas |
